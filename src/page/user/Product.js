  ...
  useEffect(() => {
    ...
    if (selectedCategory) {
      filtered = filtered.filter(
        (product) => product.category.id_category === selectedCategory
      );
    }

    if (selectedBrand) {
      filtered = filtered.filter(
        (product) => product.brand.id_brand === selectedBrand
      );
    }
  ...
  return (
    <Layout>
      <div className="text-center w-full font-black overflow-hidden mt-[65px] px-2">
        <div
          data-aos="fade-left"
          className="flex gap-6 flex-col  w-10/12  md:w-full relative mb-auto justify-end items-end  "
        >
        ...
        <div
          className="grid md:grid-cols-4 grid-cols-1 gap-4 md:w-full h-auto min-h-screen w-11/12 justify-center items-center
     ml-2 my-3 font-mono md:ml-0"
        >
           {filteredProducts.map((product) => (
              ...
              >
                ...
                >
                  <div className="flex flex-col h-[450px]">
                    ...
                    <p className="text-sm text-gray-500 pb-3">
                      Giảm Giá:{" "}
                      {parseFloat(product.discoust) === 0 ||
                      parseFloat(product.discoust) > 100
                        ? "Không có giảm giá"
                        : `${(parseFloat(product.discoust) / 100) * 100} %`}
                    </p>
                    <Rate allowHalf defaultValue={4.5} />
                    <h3 class="text-center uppercase font-bold my-2 text-gray-800  w-full  ">
                      "{product.name}"
                    </h3>
                    <h3 className="text-start text-base font-light text-gray-900">
                      Thể Loại: {product.category.name}
                    </h3>
                    <h3 className="text-start text-base font-light text-gray-900">
                      Quốc gia: {product.brand.name}
                    </h3>
                  </div>
                ...
                <div class="flex justify-around items-center  h-1/12">
                  <div class="flex gap-2 text-gray-600 hover:scale-110 duration-200 hover:cursor-pointer">
                    ...
                    <button
                      class="font-semibold text-sm text-red-700"
                  ...
                  {product.stock > 0 ? (
                    <div class="flex gap-2 text-gray-600 hover:scale-110 duration-200 hover:cursor-pointer">
                      ...
                      >
                        <path
                          d="M19.24 5.579h-.4l-3.38-3.38a.706.706 0 0 0-.99 
                            ...
                            2.03.24.25.53.38.84.45.29.07.6.08.9.08h15.28c.31 0 .6-.02.88-.08.84-.2 1.48-.8 1.48-2.48 0-2.56-1.87-2.56-2.76-2.56ZM19.09 12H4.91c-.62 0-1.09.55-.99
                             ...
                             -1.5a.754.754 0 0 1 0-1.06c.3-.29.77-.29 1.07 0l.99.99 2.72-2.51c.3-.28.78-.26 1.06.04.28.31.26.78-.05 1.06Z"
                          fill="#32bfdb"
                        ></path>
                      </svg>
                      ...
                      <button
                        ...
                        class="font-semibold text-sm text-[#32bfdb]"
                      >
                        Thêm giỏ hàng
                      </button>
                    </div>
                  ) : (
                    <h1>Đã hết hàng</h1>
                  )}
                  {selectedProduct && (
                    <Drawer
                      placement="left"
                      title={`Chi tiết sách ( ${selectedProduct.name} )`}
                    ...
                    >
                      <div className="justify-center items-center flex flex-col space-y-5">
                        <h1 class="text-center font-bold my-2 text-gray-800  w-full  ">
                        ...
                        <p className="text-sm text-gray-500 pb-3">
                          Giảm Giá:{" "}
                          {parseFloat(selectedProduct.discoust) === 0 ||
                          parseFloat(selectedProduct.discoust) > 100
                            ? "Không có giảm giá"
                            : `${
                                (parseFloat(selectedProduct.discoust) / 100) *
                                100
                        ...
                        <Image
                          ...
                          width={300}
                          height={300}
                          className="text-center justify-center items-center border-2 rounded-3xl"
                        />
            ...
            <div className="flex w-auto self-center h-screen items-center justify-center my-5 rounded-xl  bg-[#ffffffcc]">
                <div class="loader">
