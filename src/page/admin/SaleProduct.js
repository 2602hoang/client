...
function SaleProduct() {
  ...
  return (
    ...
    <div className='min-h-screen h-auto flex-col  flex md:w-full w-full mx-2  mt-[65px]'>
    <h1 className='text-center text-3xl font-bold'>Danh sách sản phẩm đang kinh doanh</h1>
    <div data-aos="fade-left" className='flex gap-6 flex-col  w-10/12  md:w-full h-auto relative justify-center items-center  '>
      ...
      my-3 font-mono md:ml-0" >
          {filteredProducts.map((product) => (
            <div
data-aos="flip-left"

              key={product.id_product}
              className="flex flex-col bg-white w-full font-mono space-y-8 h-[600px] rounded-md py-4  border-2 border-green-500"
            >

              <div
                // style={{backgroundImage: `url(${product.images })`, backgroundPosition: 'center', backgroundSize: 'cover', backgroundRepeat: 'no-repeat'}}
                class="flex flex-col bg-white w-full h-auto overflow-hidden justify-center items-center rounded-md py-4 px-6 ">
                ...
                <h3 className="text-start text-base font-light text-gray-900">Số lượng: {product.stock>0 ? product.stock : 'Hết hàng'} (quyển)</h3>
              </div>
              <div class="flex justify-around items-center  h-1/12">
                ...
                {selectedProduct && (
                <Drawer placement='left' title={`Chi tiết sách ( ${selectedProduct.name} )`} width={window.innerWidth >= 768 ? "500px" : '100%'} 
                onClose={onClose} visible={selectedProduct !== null} footer={null}>
