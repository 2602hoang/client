...
function Header({ cart }) {
  ...
  function renderItem(product, index) {
    const showName =
      index ===
      cart.findIndex((cartItem) => cartItem.id_product === product.id_product);

    return (
      <List.Item style={{ display: showName ? "block" : "none" }}>
        <List.Item.Meta
          ...
          title={
            <div className=" flex justify-between items-start h-min-screen h-auto">
              <div className="flex flex-col justify-start items-start h-auto">
                <h1 className="text-red-700 font-mono font-bold text-start mb-5 ">
                  {" "}
              ...
              <div className="flex  flex-col space-y-6 justify-end items-end ">
                ...
                <h1 className="font-mono text-black mt-24">
                  TT:{" "}
                  {formatCurrency(
          ...
          description={<div className="flex justify-end items-end"></div>}
        />
      </List.Item>
    );
  }
  ...
  return (
    <div
      className=" justify-center flex fixed   items-center font-mono 
                 flex-row bg-[#539ba9]  h-[60px] md:w-full z-40 w-full top-0 "
    >
      <nav className=" justify-between items-center py-2 flex   w-full max-w-6xl mx-2">
        <div className="md:flex top-0 left-0 right-0  z-50  md:z-50   md:w-1/5 flex flex-row md:mr-auto ">
          <a
            href="/"
            className="font-black flex  items-center font-sans gap-x-2 md:ml-20 bg-clip-text text-transparent 
          bg-gradient-to-r from-red-700 to-yellow-400"
          >
            <Image
              src={background}
              className=" space-x-1 h-12 w-12 rounded-full"
              width={50}
              height={50}
            />
            User
        ...
        >
          <li className="relative md:m-0">
            <a
              href="/home"
              className=" text-white text-sm hover:border-b-2 hover:animate-ping
          ...
          <li className=" md:m-0 relative text-center">
            ...
            <ul
              className={`md:absolute relative md:text-start text-center ${
                subMenuVisible
                  ? "mt-3 opacity-80  rounded-br-xl rounded-bl-xl"
                  : "hidden"
              }`}
            >
              <div className="md:w-[200px] text-sm flex flex-col rounded-br-lg rounded-bl-lg space-y-4 mt-2 w-full p-2 md:bg-[#539ba9] bg-sky-300 text-white  ">
                <li>
                  <a href="/order">Đơn chưa xác nhận TT</a>
                </li>
          ...
          <li className=" md:m-0 ">
            <a
              href="/contact"
              className=" text-white text-sm hover:border-b-2 hover:animate-ping text-center
                         hover:border-red-500 hover:text-red-500
                          transition duration-500 ease-in"
            >
          ...
          <li className=" md:m-0 relative text-center">
            <a
              ...
              className="text-white   hover:text-red-500 justify-center transition duration-500 ease-in"
            >
              {user.avatar !== null ? (
                <Avatar alt="" src={user.avatar} className="size-9" />
              ) : (
                <Avatar>
                  {user.id_role === 123
                    ? "QL"
            ...
            <ul
              className={`md:absolute relative md:text-start text-center justify-center items-center  ${
                subMenuVisible1
                  ? "mt-3 opacity-80  rounded-br-xl rounded-bl-xl"
                  : "hidden"
              }`}
            >
              <div
                className={` w-full md:w-[200px] flex flex-col space-y-4 rounded-br-lg rounded-bl-lg mt-1 p-2 mr-[200px] md:bg-[#539ba9] bg-sky-300 text-white text-center`}
              >
                <li className="my-5 md:m-0 relative text-center">
                  <a href={`/user?userId=${userId}`}>
                    {" "}
                    <UserOutlined />
                    Thông tin cá nhân
                  </a>
        ...
        <div
          id="menu-button"
          className="absolute  top-5 right-5  md:hidden cursor-pointer z-50 flexd"
          onClick={toggleMenu}
        >
      ...
      <div>
        ...
        <Modal
          title={
            <div className="flex  justify-center items-center gap-5 h-min-screen h-auto ">
        ...
        >
          <List
            ...
            header={
              <div className=" flex justify-start items-start hover:animate-bounce ">
              ...
              <div classsName="w-full flex-row  ">
                ...
                <div className=" flex-col md:w-1/3 justify-end items-end ml-auto flex">
                  {cart.reduce((total, item) => total + item.price, 0) > 0 ? (
                    <h1 className="justify-end items-end text-black font-bold text-end">
                      {" "}
                      Tổng tiền hóa đơn:{" "}
                      {formatCurrency(
