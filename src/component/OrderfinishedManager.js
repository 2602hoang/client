...
function OrderfinishedManager() {
  ...
  const showModal = async (id_order) => {
    ...
    try {
      const response = await axios.get(
        `${URL}api/v1/order/getonebyOrderId/${id_order}`,
        {
          headers: {
  ...
  const getOrders = async () => {
    try {
      const response = await axios.get(
        `${URL}api/v1/order/getall/status1/idpay25`,
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
          },
  ...
  useEffect(() => {
    ...
    if (filterConfirmed) {
      filtered = orders.filter(
        (order) => order.id_pay === 2 && order.finished === 1
      );
    } else if (filterNotConfirmed) {
      filtered = orders.filter(
        (order) =>
          order.id_pay === 3 || (order.id_pay === 5 && order.finished === 1)
      );
    } else if (searchTerm) {
      filtered = orders.filter((order) =>
        order.user.phone.toString().includes(searchTerm)
      );
    } else if (filterAll) {
  ...
  return (
    <div className="flex flex-col w-full h-auto min-h-screen my-9">
      <div
        data-aos="fade-down"
        className="flex flex-col md:w-full justify-center items-center"
      >
                    ...
                    <td className="border border-slate-300  ">
                      ...
                      <Tooltip title="Xem chi tiết đơn">
                        <div className="text-center items-center justify-center flex flex-col">
                          <Button
                            onClick={() => showModal(order.id_order)}
                            type="link"
                            title="Xem"
                    ...
                    <td className="border border-slate-300">
                      <div className="flex flex-col space-y-4 m-2">
                        <TextArea
                          ...
                          disabled
                          placeholder={
                            order.note_pays?.length > 0 ? order.note_pays : ""
                          }
      ...
      <Modal
        ...
        title={
          <div className="flex row justify-center items-center space-x-4">
            <h1 calssName="text-center justify-center items-center">
      ...
      >
        {selectedOrderDetails?.map((order) => (
          ...
          >
            <div
              id="pdf-content"
              className="w-full mt-auto flex justify-center items-center h-auto"
            >
              <div
                className={`border-2 border-gray-800 flex-row w-[380px] overflow-hidden 
                ${
                  order.id_pay === 1
                    ? "border-yellow-300"
                    : order.id_pay === 2
              ...
              >
                ...
                <p className="border-b-2 mb-2 w-full">
                  Ghi chú: {order.notes ? order.notes : "không có ghi chú"}
                </p>
                <p className="text-black font-black text-xl font-mono text-center">
                  {" "}
                  CHi tiết hóa Đơn{" "}
                </p>
                {Object.keys(prodcut)?.map((id, index) => (
                  <div
                    key={index}
                    className="mb-2  font-mono  flex-row border-b-2 border-dashed my-2"
                  >
                    <p className="font-semibold text-[#214f32]">
                      (SP{index + 1}) "{prodcut[id].name}"
                    </p>
                    <img src={prodcut[id].images} className="w-16 h-16" />
                    <p className="text-end">SL: {prodcut[id].count}</p>
                    <p className="text-start">
                      Giá: {formatCurrency(prodcut[id].price)}
                    </p>
                    <p className="text-end mb-2">
                      Thành tiền:{" "}
                      {formatCurrency(prodcut[id].price * prodcut[id].count)}
                    </p>
                  </div>
                ))}
                <p className="border-b-2 mb-2 w-full text-end">
      ...
      <Modal
        ...
        footer={[
          <Button
            key="hoanthanh"
            onClick={() => hendleXacnhan(selectedOrderId)}
          >
            Hoàn Thành
          </Button>,
