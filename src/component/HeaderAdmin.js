    ...
    flex-row bg-[#539ba9]  h-[60px] md:w-full z-40 w-full top-0 '>
             ...
             <nav className='overflow-hidden justify-start items-center py-2 flex md:ml-6 md:w-full w-full mx-2'>
                <div className='md:flex top-0 left-0 right-0  z-50  md:z-50   md:w-1/5 flex flex-row md:mr-auto  '>
                    <a href='/productmanager' className='font-black flex  items-center gap-x-2 bg-clip-text text-transparent bg-gradient-to-r from-red-700 to-yellow-400'>
            ...
            md:space-y-0
               bg-sky-300
                fixed z-20 top-0 left-0 w-full h-full flex font-bold flex-col justify-center items-center  ${menuVisible ? 'overflow-hidden h-screen overflow-y-visible fixed w-full' : 'invisible'
                    ...
                    <li className='my-5 md:m-0'>
                        ...
                        <li className='my-5 md:m-0'>   
                <svg onClick={() => {handlelogout(); nav("/admin")}} xmlns="http://www.w3.org/2000/svg" width="32" height="32"
